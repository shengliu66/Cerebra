...
def validate_structured_output_model(model_string: str):
    ...
    Structure_Output_Models = ["gpt-4o", "gpt-4o-mini", "gpt-4.1", "gpt-4.1-mini", "gpt-4.1-nano", "o3-mini-2025-01-31", "gpt-5-2025-08-07"]
...
def validate_web_search_model(model_string: str):
    return any(x in model_string for x in ["gpt-5-search-api", "gpt-4o-search-preview", "gpt-4o-mini-search-preview"])
...
class ChatOpenAI(EngineLM, CachedEngine):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        :param system_prompt:
        :param is_multimodal:
        :param use_nyu_hipaa: Whether to use NYU HIPAA GPT service
        """
        ...
        self.is_web_search_model = validate_web_search_model(self.model_string)
        if self.use_cache:
    ...
    def _generate_text(
        ...
        if self.is_web_search_model:

            response = self.client.responses.parse(#.chat.completions.create(
                model="gpt-5",
        ...
        elif self.is_chat_model and self.support_structured_output and response_format is not None:
